...
export default function Sidebar() {
    ...
    const { glossary, lookupWord, lookupWordFull, externalDictionaries, dictionariesLoading, toggleDictionary } = useGlossary();
    ...
    const SectionHeader = ({ icon, title, badge }: { icon: string; title: string; badge?: number }) => (
    ...
    return (
        <div className="w-72 bg-white/60 dark:bg-slate-900/60 backdrop-blur-md border-l border-slate-200 dark:border-slate-700 overflow-y-auto flex flex-col shadow-sm">
            ...
            {/* Conflict Monitor */}
            <div className="p-4">
                <SectionHeader icon="⚠️" title={t('conflictMonitor')} badge={unresolvedConflicts.length} />
            ...
            {/* Dictionary Lookup */}
            <div className="p-4">
                <SectionHeader icon="📚" title={t('dictionaryLookup')} />
                                        ...
                                        <div key={idx} className={`${idx > 0 ? 'pt-2 border-t border-slate-200 dark:border-slate-700' : ''}`}>
                                            ...
                                            <div className="flex flex-wrap gap-1 mb-1">
                                                <span className={`inline-block text-[9px] px-1.5 py-0.5 rounded-full font-semibold ${match.sourceType === 'manual'
                                                    ...
                                                    {match.sourceType === 'manual' ? '📖' : match.sourceType === 'auto' ? '🤖' : '📚'} {match.sourceName}
                                                </span>
                                                {match.pos && (
                                            ...
                                            {(match.count !== undefined && match.count > 0) && (
                                                <div className="text-[10px] text-slate-400 dark:text-slate-500">
                                                    Seen {match.count}x
                                                    {match.isAmbiguous && (
                                                        <span className="ml-1 text-amber-600 dark:text-amber-400">⚠ {t('ambiguous')}</span>
                                                    )}
                                                </div>
                                            )}
                                        </div>
                ...
                {(externalDictionaries.length > 0 || dictionariesLoading) && (
                    <div className="mt-4 pt-3 border-t border-slate-200 dark:border-slate-700">
                        <div className="text-[10px] font-semibold text-slate-600 dark:text-slate-400 mb-2 uppercase tracking-wide flex items-center gap-1">
                            📖 Dictionaries ({externalDictionaries.length})
                            {dictionariesLoading && (
                                <span className="inline-block w-3 h-3 border-2 border-blue-400 border-t-transparent rounded-full animate-spin ml-1" />
                            )}
                        </div>
                        <div className="space-y-1.5">
                            {externalDictionaries.map((dict) => (
                                <div
                                    key={dict.id}
                                    className={`rounded-lg p-2.5 shadow-sm border transition-all duration-200 ${dict.enabled
                                        ? 'bg-gradient-to-br from-blue-50 to-blue-100/50 dark:from-blue-900/30 dark:to-blue-900/10 border-blue-100 dark:border-blue-800/50'
                                        : 'bg-slate-50 dark:bg-slate-800/40 border-slate-200 dark:border-slate-700 opacity-60'
                                        }`}
                                >
                                    <div className="flex items-center justify-between">
                                        <div className="text-xs font-semibold text-blue-800 dark:text-blue-200 truncate flex items-center gap-1 flex-1 min-w-0">
                                            <span>{dict.enabled ? '📚' : '📕'}</span>
                                            <span className="truncate">{dict.nameMyanmar || dict.name}</span>
                                        </div>
                                        <button
                                            onClick={() => toggleDictionary(dict.id)}
                                            className={`relative w-8 h-4 rounded-full transition-colors duration-200 flex-shrink-0 ml-2 ${dict.enabled
                                                ? 'bg-blue-500'
                                        ...
                                        >
                                            <span
                                                className={`absolute top-0.5 w-3 h-3 bg-white rounded-full shadow transition-transform duration-200 ${dict.enabled ? 'translate-x-4' : 'translate-x-0.5'
                                                    }`}
                                            />
                                        </button>
                                    </div>
                                    <div className="flex items-center gap-1.5 mt-1">
                                        <span className={`text-[9px] px-1.5 py-0.5 rounded-full font-medium ${dict.category === 'general' ? 'bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-300' :
                                            dict.category === 'phrases' ? 'bg-purple-100 text-purple-700 dark:bg-purple-900/40 dark:text-purple-300' :
                                                dict.category === 'transliteration' ? 'bg-orange-100 text-orange-700 dark:bg-orange-900/40 dark:text-orange-300' :
                        ...
                        <div className="text-[10px] text-slate-400 mt-2 text-center">
                            {externalDictionaries.filter(d => d.enabled).reduce((sum, d) => sum + d.entryCount, 0).toLocaleString()} active entries
                        </div>
                    </div>
                )}
            </div>
            ...
            <div className="p-4">
                <SectionHeader icon="⚙️" title={t('granularitySettings')} />
            ...
            {/* Keyboard Shortcuts */}
            <div className="p-4">
                <SectionHeader icon="⌨️" title={t('shortcuts')} />
                <div className="space-y-1">
                    {[
